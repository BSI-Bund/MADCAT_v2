...
long int worker_tcp(char* dst_addr, int dest_port, char* src_addr, int src_port, long double timeout, \
                    char* data_path, int max_file_size, int s, FILE* confifo)
{
...
\"payload_sha1\":\"%s\"\
    ...
    char* tmp_str = json_do(false, "");
    fprintf(confifo,"%s\n", tmp_str); //print json output for further analysis
    fflush(confifo);
#if DEBUG >= 2
