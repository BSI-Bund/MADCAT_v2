...
{
    struct iphdr *iphdr = (struct iphdr *)(packet + ETHERNET_HEADER_LEN);
    if(caplen - (ETHERNET_HEADER_LEN + iphdr->ihl*4 + IP_OR_TCP_HEADER_MINLEN) < 0) return -1; //Malformed Paket
    ...
    long int data_bytes = caplen - (ETHERNET_HEADER_LEN + iphdr->ihl*4 + tcphdr->doff*4);
